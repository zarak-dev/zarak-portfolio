...
import { Bot, X, Send, Sparkles, Loader2, User, Briefcase, FolderGit2, Wrench, Github, Mail } from 'lucide-react';
...
export default function AskZarak({ onOpenXRay, initialContextMessage, onClearContextMessage }: AskZarakProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', content: "Hi! I'm Zarak AI. I can answer questions about Zarak's engineering career, projects, and technical skills. What would you like to know?" }
  ]);
  ...
  const handleSubmit = async (e?: React.FormEvent, overrideInput?: string) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || 'Zarak AI is temporarily unavailable.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <AnimatePresence>
        {!isOpen && (
          ...
          >
            ...
            <span className="font-mono text-sm font-bold tracking-tight">ASK ZARAK</span>
          </motion.button>
        )}
      </AnimatePresence>
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-border/70 bg-secondary/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-[10px] text-muted-foreground">ENGINEERING CONTEXT SYSTEM</p>
                  </div>
                </div>
                <button
                  onClick={() => setIsOpen(false)}
                ...
                {error && (
                  <div className="p-4 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive font-mono text-xs text-center space-y-3">
                    ...
                    <div className="flex flex-wrap justify-center gap-2 mt-2">
                      <a href="#projects" onClick={() => setIsOpen(false)} className="px-3 py-1.5 rounded-lg bg-background border border-border hover:bg-secondary transition-colors text-foreground">Explore Projects</a>
                      <a href="#experience" onClick={() => setIsOpen(false)} className="px-3 py-1.5 rounded-lg bg-background border border-border hover:bg-secondary transition-colors text-foreground">View Experience</a>
                    </div>
                  </div>
                )}
              ...
              <div className="p-4 bg-secondary/20 border-t border-border/70 space-y-3">
                ...
                <div className="flex justify-center gap-4 text-muted-foreground mb-1">
                  <button onClick={() => handleQuickAction("Tell me about your career experience.")} className="hover:text-accent transition-colors" title="Experience"><Briefcase className="w-4 h-4" /></button>
                  <button onClick={() => handleQuickAction("What projects have you built?")} className="hover:text-accent transition-colors" title="Projects"><FolderGit2 className="w-4 h-4" /></button>
                  <button onClick={() => handleQuickAction("What is your tech stack?")} className="hover:text-accent transition-colors" title="Tech Stack"><Wrench className="w-4 h-4" /></button>
                  <button onClick={() => handleQuickAction("Show me your GitHub repositories.")} className="hover:text-accent transition-colors" title="GitHub"><Github className="w-4 h-4" /></button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="relative">
                  <input
                    ...
                    placeholder="Ask about Zarak's engineering work..."
                    disabled={isLoading}
