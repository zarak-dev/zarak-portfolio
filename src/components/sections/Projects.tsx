...
export default function Projects({ onOpenXRay, onOpenCaseStudy }: ProjectsProps) {
  ...
  const handleToggleExpand = () => {
    if (!isExpanded) {
      setIsExpanded(true);
      setTimeout(() => {
        const el = document.getElementById('projects-expanded-content');
        if (el) {
          const yOffset = -80;
  ...
  const handleProjectClick = (projectTitle: string) => {
    setIsExpanded(true);
    const targetId = `project-${projectTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
    setTimeout(() => {
      ...
      if (el) {
        ...
        const fallback = document.getElementById('projects-expanded-content');
        if (fallback) {
          const yOffset = -80;
          const y = fallback.getBoundingClientRect().top + window.pageYOffset + yOffset;
          window.scrollTo({ top: y, behavior: 'smooth' });
        }
      }
    }, 100);
  ...
  return (
    <section id="projects" className="border-b border-line bg-surface-2 py-20 sm:py-28">
      <div className="shell">
        ...
        <div className="relative mt-10 overflow-hidden rounded-3xl border border-line bg-surface/85 backdrop-blur-xl p-4 sm:p-8 shadow-[0_12px_40px_-20px_rgba(0,0,0,0.35)] transition-all duration-300 dark:border-white/10 dark:bg-surface-2/90">
          ...
          <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <button
              ...
              className="group relative inline-flex min-h-[48px] w-full sm:w-auto shrink-0 items-center justify-center gap-2.5 rounded-full bg-ink px-6 py-3.5 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:bg-brand hover:shadow-md active:scale-[0.98] dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100 touch-manipulation cursor-pointer"
            >
              <span>
                {isExpanded
              ...
              <ChevronDown
                className={cn(
                  'h-4 w-4 shrink-0 transition-transform duration-300 group-hover:translate-y-0.5',
          ...
          <div className="relative z-10 mt-6 border-t border-line/70 pt-5 dark:border-white/10">
            <div className="mb-3 flex items-center justify-between font-mono text-xs text-ink-faint">
              ...
              <span className="inline font-mono text-[11px] text-brand sm:hidden">
                Swipe 5 systems →
              </span>
              <span className="hidden font-sans text-[11px] font-medium text-ink-mute dark:text-ink-soft sm:inline">
                {isExpanded ? 'Click collapse button or card to close' : 'Click any card to inspect full architecture'}
              </span>
            </div>

            <div className="flex overflow-x-auto gap-3 pb-2 pt-1 px-0.5 snap-x snap-mandatory no-scrollbar sm:grid sm:grid-cols-3 lg:grid-cols-5 sm:overflow-visible sm:pb-0">
              {PROJECTS.map((project) => (
                <button
                  key={project.title}
                  type="button"
                  onClick={() => handleProjectClick(project.title)}
                  className="group/card flex w-[210px] min-w-[210px] max-w-[210px] sm:w-auto sm:min-w-0 sm:max-w-none shrink-0 sm:shrink flex-col overflow-hidden rounded-xl border border-line bg-surface-2 p-2.5 text-left transition-all duration-200 hover:-translate-y-1 hover:border-brand-line hover:shadow-md dark:border-white/10 dark:bg-surface touch-manipulation cursor-pointer active:scale-95 snap-start"
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-1 border-b border-line/40 pb-1.5 dark:border-white/5">
                    ...
                    <span className="truncate font-mono text-[9px] font-medium text-ink-faint">
                      {project.year}
                    </span>
                  </div>
                  ...
                  <div className="mt-auto pt-1">
                    ...
                    <span className="mt-1 inline-flex items-center gap-1 font-mono text-[9px] font-semibold text-brand group-hover/card:underline">
                      Inspect System →
                    </span>
                  </div>
                </button>
            ...
            >
              ...
              <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-b border-line pb-4 dark:border-white/10">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-surface px-3.5 py-1.5 font-mono text-xs font-medium text-ink-mute transition-colors hover:border-brand-line hover:bg-brand-soft hover:text-brand dark:border-white/10 dark:bg-white/[0.04] touch-manipulation active:scale-95 cursor-pointer"
                >
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {project.xrayId && onOpenCaseStudy && (
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-surface-2 px-3 py-1.5 font-mono text-[11px] font-medium text-ink-mute transition-colors hover:border-brand-line hover:bg-brand-soft hover:text-brand cursor-pointer"
                            >
              ...
              <div className="mt-12 flex flex-col items-center justify-center gap-3 border-t border-line/70 pt-8 dark:border-white/10">
                ...
                <button
                  type="button"
                  onClick={handleCollapse}
                  className="inline-flex items-center gap-2 rounded-full border border-line bg-surface px-5 py-2.5 text-xs font-semibold text-ink shadow-sm transition-all hover:border-brand hover:text-brand dark:border-white/10 dark:bg-white/[0.04] cursor-pointer"
                >
